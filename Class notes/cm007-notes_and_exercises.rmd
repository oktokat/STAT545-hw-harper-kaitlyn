...
gapminder %>% 
  ggplot(aes(x=year, y=lifeExp)) +
  geom_smooth(method=lm, se=FALSE, aes(group=continent), color = "black") +
  geom_point(aes(color=continent)) 

### the regression line is behind the points (the points are the front layer)
...
### this is because we still have the input x and y values. We don't need the points to make the line
...
- `facet_wrap`: 1D facetting -- group by one variable
- `facet_grid`: 2D facetting -- group by two variables
...
### NOTE: geom_smooth(span=0.75) is the default
### span makes your line wigglier or smoother
...
### fig.width = 8 --> this makes the figure wider than the original
...
rm(vc2)
### this mutate function turns the variable into a column of 0's and 1,s
### i.e. if the pop is greater than 7m, it is a 1; if not, it's a 0

test = c("hi", "hi2")
test[c(1, 2, 2, 1)]

test=gapminder[1,1:3]
...
### NOTE: you can also change this by assigning color to the country; this automatically groups them! 

ggplot(gapminder, aes(year, lifeExp)) +
    geom_line(alpha=0.2, aes(color=country), show.legend = F)
...
```{r geom_path, echo=TRUE}

gapminder %>%
  ggplot(aes(x=year, y=pop, group=country)) +
  geom_line(aes(color=gdpPercap), alpha=0.1)

```

__Exercise 6__: Add points to the plot in Exercise 5.

```{r add_points, echo=TRUE}

gapminder %>%
  ggplot(aes(x=year, y=pop, group=country)) +
  geom_jitter(aes(color=gdpPercap), alpha=0.1, color="red") +
  geom_line(aes(color=gdpPercap), alpha=0.1)
