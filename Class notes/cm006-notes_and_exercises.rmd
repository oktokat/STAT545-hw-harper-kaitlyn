...
```{r}
ggplot(gapminder, aes(x=year, y=lifeExp)) +          # aes = "aesthetics"!!!!!
    geom_point()                                     
```
...
```{r changePointSize, echo=TRUE}

p2= gapminder %>%
  ggplot(aes(x=log(gdpPercap), y=lifeExp, size=year))+
...
3. To `p2`, colour the points by `continent`, with year being represented by the size of the points.

```{r gdp_LE, echo=TRUE}

p2= gapminder %>%
  ggplot(aes(x=log(gdpPercap), y=lifeExp, size=year, color = continent))+
  geom_point()
...
p2= gapminder %>%
  ggplot(aes(x=gdpPercap, y=lifeExp, size=year, color = continent)) +
  geom_point() +
  scale_x_log10()
