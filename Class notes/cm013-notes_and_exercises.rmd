...
### tibble allows you to back-reference (e.g. reference the column "x" in making y -- data.frame does NOT do that!)
...
(p1 <- gapminder %>% 
        filter(year > 1990) %>%
        ggplot(aes(x=year)) +
        geom_bar(aes(weight=gdpPercap, fill=continent), # for barplots, weight is used instead of "y"
                 position="dodge",
                 colour="black") +
...
```{r}
my_cols <- c("red", "blue", "green", "wheat", "orange")
...
```{r}
p3 + scale_colour_gradient2(low="azure4", mid="white", high="red4")
```
...
```{r lifeExp_coloured, echo=TRUE}

gapminder %>% 
ggplot(aes(x=lifeExp, fill=continent)) +
  geom_histogram(aes(y=..density..)) +     #### ..density.. will give you the proportion instead of the count!!!!
...
gapminder %>% 
  ggplot(aes(x=year, y=lifeExp, alpha=0.2)) +
  geom_line(aes(group=country)) +
  geom_line(data=gapminder[which(gapminder$country=="Rwanda"),], aes(color="red", size=2))




colour_layer = scale_colour_manual("",                                           ### NO LEGEND TITLE
...
gapminder %>%
  ...
  scale_alpha_discrete(range=c(0.2, 1),
                       guide=FALSE) ### GETS RID OF SECOND LEGEND
