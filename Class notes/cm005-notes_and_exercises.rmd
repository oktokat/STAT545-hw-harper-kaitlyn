...
gapminder %>%
  group_by(country) %>% 
  summarize(minGPD = min(gdpPercap))
...
```{r years_of_record, echo=TRUE}

gapminder %>% 
  group_by(country) %>% 
  summarize(yearCount = length(country))
